...
$app->post('/post', function($request, $response, $args) use ($app){
    ...
    $message->execute(['hash' => $hash, 'message' => $params['message']]);
    ...
    $this->mail->sendMessage($this->config->get('services.mailgun.domain'),
            [
                ...
                'html'=>$this->view->fetch('email/message.twig',['hash'=>$hash])
                ]);
...
$app->get('/message/{hash}', function ($request, $response, $args) {
    ...
    $message->execute(['hash'=> $hash]);
    $message= $message->fetch(PDO::FETCH_OBJ);
    
    return $this->view->render($response,'/message/show.twig', ['message'=>$message]);  
})->setName('message');